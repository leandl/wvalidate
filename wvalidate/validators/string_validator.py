...
class StringValidator(Validator):
  ...
  def __init__(
    ...
    if min is not None and (not isinstance(min, int) or isinstance(min, bool) or min < 0):
      raise StringValidatorException("The \"min\" property must be an instance of int and greater than or equal to zero.")
    
    if max is not None and (not isinstance(max, int) or isinstance(max, bool) or max < 0):
    ...
    if regex is not None and not isinstance(message_error_invalid_regex, str):
  ...
  def is_valid(self, data) -> ValidatorReturn:
    ...
    if self.__equal is not None and self.__equal != data:
